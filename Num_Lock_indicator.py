...
class NumLockIndicator:
    ...
    def create_icon_image(self, is_num_on):
        ...
        if is_num_on:
            text = "123"  # 数字模式
            font = self.font_small  # 较宽文字用小字号
        else:
            text = "arr"  # 方向键模式 (arrow 缩写)
            font = self.font_large  # 较窄文字用大字号
    ...
    def update_icon(self):
        ...
        if state != self.current_state:
            ...
            if self.icon:
                ...
                self.icon.title = "Num Lock: 123 (数字模式)" if state else "Num Lock: arr (方向键模式)"
    ...
    def run(self):
        ...
        self.icon = pystray.Icon(
            "numlock_indicator",
            initial_icon,
            "Num Lock: 123 (数字模式)" if initial_state else "Num Lock: arr (方向键模式)",
            menu
        )
